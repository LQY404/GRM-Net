...
class StandardROIHeadsRef(ROIHeads):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.fusion_choice == 1:
            ...
            self.concat_dim = 2 * self.roi_dim + self.rel_pos + self.topk*self.rel_pos
        ...
        self.use_gate = False
        if self.use_gate:
            self.lfc3 = nn.Sequential(
                nn.Linear(self.hn_dim, 1024),
                # nn.BatchNorm1d(self.roi_dim),
                # nn.GroupNorm(32, self.roi_dim),
                nn.ReLU(),
                nn.Dropout(0.1),
    ...
    def _fusion1(self, features, proposals, hn, images=None):
        ...
        if self.add_abs and self.topk > 0:    
            print(box_features.shape, hn2.shape, abs_pos_per_img.shape, rel_pos_per_img.shape)
        elif not self.add_abs and self.topk > 0:
            print(box_features.shape, hn2.shape, rel_pos_per_img.shape)
        elif self.add_abs and self.topk == 0:
            print(box_features.shape, hn2.shape, abs_pos_per_img.shape)
        else:
            print(box_features.shape, hn2.shape)
        
        # box_features = F.normalize(box_features, p=2, dim=1)
        # box_features = torch.cat((box_features, hn, abs_pos_per_img), dim=1) # [N*256, 5+hn_dim+256, 7, 7]
        if self.add_abs and self.topk > 0:
            box_features = torch.cat((box_features, hn2, abs_pos_per_img, rel_pos_per_img), dim=1) # [N*256, 5+hn_dim+256, 7, 7]
        elif not self.add_abs and self.topk > 0:
            box_features = torch.cat((box_features, hn2, rel_pos_per_img), dim=1)
        elif self.add_abs and self.topk == 0:
            box_features = torch.cat((box_features, hn2, abs_pos_per_img), dim=1)
        else:
            box_features = torch.cat((box_features, hn2), dim=1)
        ...
        if self.use_gate:
            hn3 = self.lfc3(hn).unsqueeze(1)
            box_features = box_features.reshape(N, -1, 1024)
            
            box_features1 = self.vlc(box_features) # [N, -1, 1024]
            box_features1 = box_features1.permute(0, 2, 1)  # [N, 1024, -1]
            
            gate = torch.bmm(hn3, box_features1) #[N, 1, -1]
            gate = gate.squeeze(1) # [N, -1]
            
            # 添加监督
            
            #########
            gate = F.sigmoid(gate).unsqueeze(-1)
            box_features = box_features + gate*box_features
    ...
    def _fusion11(self, features, proposals, hn, images=None):
        N, C, vh, vw = features[-1].shape
        ...
        if self.use_gate:
            ...
            box_features1 = box_features.permute(0, 2, 1)  # [N, 1024, -1]
            ...
            box_features = box_features + gate*box_features
            box_features = box_features.reshape(-1, 1024)
            box_features = self.vlc(box_features) 
