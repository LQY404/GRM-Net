...
def build_resnet_sketch_backbone(cfg, input_shape):
    """
    ...
    # rate = [1, 2, 5, 1, 2, 5] if cfg.HDC else [1, 1, 1, 1, 1, 1]
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        if cfg.MODEL.NUM_CLASSES == 46:
            print("使用空洞卷积")
            # dilation = 2**(stage_idx - 1)
            # dilation = 6*(stage_idx - 1)
        # dilation = max(16, 2**(stage_idx - 1))
        # dilation = 2 if stage_idx == 5 else 1
        # dilation = rate[idx] 
...
@BACKBONE_REGISTRY.register()
def build_resnet_sketch_fpn_backbone(cfg):
    """
    Args:
    ...
    bottom_up = build_resnet_sketch_backbone(cfg, input_shape)
...
#     r = build_resnet_sketch_backbone(cfg, input_shape)
