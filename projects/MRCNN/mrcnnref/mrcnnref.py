...
class MRCNNRef(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        bidirectional = 2
        self.hn_dim = self.rnn_dim * bidirectional if (not self.use_bert and not self.use_roberta) else 768
        ...
        self.save_path = "/home/lingpeng/project/Adet_2022/effe_RPN_pred_in_"  + ("test/" if not self.training else "train/") + cfg.RPN_SAVE + "_reltop10_heal_sketch_wodilation_512_alldata_oriconfig"
        ...
        if (self.use_clip or self.use_bert or self.use_roberta) and self.freeze_bert and self.training:
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "proposal_generator": build_proposal_generator_ref(cfg, backbone.output_shape()),
    ...
    def visualize_training(self, batched_inputs, proposals, stage='ROI'):
        """
        ...
        for input, prop in zip(batched_inputs, proposals):
            ...
            v_gt = v_gt.overlay_instances(
                boxes=input["instances"].gt_boxes.tensor.cpu().numpy(),
                labels=input['instances'].gt_classes.cpu().numpy()
            )
            anno_img = v_gt.get_image()
            box_size = min(len(prop.proposal_boxes), max_vis_prop)

            # bs = prop.objectness_logits.data.detach().cpu().numpy()
            if stage == 'ROI':
                cs = prop.cate_scores.data.detach().cpu().softmax(-1).argmax(-1).numpy()
            # print(prop.objectness_logits)
            # print(prop.cate_scores)
            # print(cs)
            # print(prop.cate_scores.data.detach().cpu().softmax(-1).argmax(-1))
            # labels = [str(b) + ": " + str(c) for b, c in cs]
            v_pred = Visualizer(img, None)
            v_pred = v_pred.overlay_instances(
                boxes=prop.proposal_boxes[0:box_size].tensor.cpu().numpy(),
                # labels=prop.gt_classes[0:box_size].cpu().numpy()
                # labels=cs[0:box_size]
                labels=[str(b) + ":" + str(c) for b, c in zip(prop.gt_classes[0:box_size].cpu().numpy(), cs[0:box_size])] if stage == 'ROI' else prop.objectness_logits[0:box_size].cpu().sigmoid().numpy()
            )
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            return MRCNNRef._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
