...
from .roi_heads_ref import GATNet, select_foreground_proposals, info_nce
...
class MGCN(nn.Module):
    ...
    def __init__(
        ...
        self.textDim = 1024 if not self.use_bert else 256
        # self.textDim = 256
        ...
        self.roi_hn_mapping = nn.Sequential(
            ...
            nn.BatchNorm1d(self.roi_hidden_dim),
            nn.ReLU(),
        )
        
        self.node_hn_mapping = nn.Sequential(
            nn.Linear(self.textDim, self.roi_hidden_dim),
            nn.BatchNorm1d(self.roi_hidden_dim),
            nn.ReLU(),
        )
        self.roi_node_mapping = nn.Sequential(
        ...
        self.roi_mapping = nn.Sequential(
            # nn.Linear(self.roi_dim, self.roi_hidden_dim),
            nn.Conv2d(self.visual_dim, self.roi_hidden_dim, 1, bias=False),
            nn.BatchNorm2d(self.roi_hidden_dim),
            nn.ReLU(),
        )
        self.combined_node_dim = self.roi_hidden_dim + 5 + self.roi_hidden_dim + self.roi_hidden_dim
        
        self.node_mapping = nn.Sequential(
            # nn.Linear(self.combined_node_dim, self.roi_hidden_dim),
            nn.Conv2d(self.combined_node_dim, self.roi_hidden_dim, 1, bias=False),
            nn.BatchNorm2d(self.roi_hidden_dim),
            nn.ReLU()
        )
        ...
        if self.use_gcn:
            ...
            if not self.use_one_gcn:
                self.graphNets = nn.ModuleList( GATNet(self.graph_in_channels, self.graph_out_channels, nheads=self.graph_head) for _ in range(self.Rstep))
        ...
        self.save_path = "/home/lingpeng/project/Adet_2022/effe_RPN_pred_in_train/" + \
                                ...
                                ("_wogcn4" if not self.use_gcn else "") + \
                                ("onegraph5" if self.use_one_gcn else "") \
                                + ("_woadj" if self.woadj else "wadj") \
                                + "_dilationbk_v3" \
                                + ("_rnn" if not self.use_bert else "")
         
        # self.save_path = "/home/lingpeng/project/Adet_2022/ocupation_test"                       
                                # ("_woaligmentloss")
    ...
    def inference(self, batched_inputs, do_postprocess: bool = True):
        ...
        if not self.use_film:
            if self.test2:
                ...
                for feat_name, feat in features.items():
                    ...
                    visu_feat[feat_name] = F.normalize(self.joint_fusion[feat_name](feat), p=2, dim=1)
        ...
        abs_pos_per_img = abs_pos_per_img.reshape(-1, 5, 1, 1).repeat(1, 1, roi_feature.shape[-2], roi_feature.shape[-1])
        ...
        final_state = final_state.reshape(roi_feature.shape[0], -1, 1, 1).repeat(1, 1, roi_feature.shape[-2], roi_feature.shape[-1])
        
        globalFeature = globalFeature.repeat_interleave(repeats=roi_num, dim=0)  # [N*B, roi_hidden_dim]
        globalFeature = globalFeature.reshape(roi_feature.shape[0], -1, 1, 1).repeat(1, 1, roi_feature.shape[-2], roi_feature.shape[-1])
        
        
        # mapping to the space same as the text embedding
        roi_feature = torch.cat((roi_feature, abs_pos_per_img, globalFeature, final_state), dim=1)  # [N*B, 3*roi_hidden_dim+5]
        roi_feature = self.node_mapping(roi_feature) # [N*B, roi_hidden_dim]
        roi_feature = F.normalize(roi_feature, p=2, dim=1)
        
        roi_feature = self.roi_heads.box_head(roi_feature) # [N*B, roi_hidden_dim]
        
        if not self.use_gcn:
            roi_feature = roi_feature.reshape(-1, self.roi_dim)
            # roi_feature = self.node_mapping_back(roi_feature)
            # roi_feature = F.normalize(roi_feature, p=2, dim=1)
            
            results, _ = self.roi_excute(roi_feature, features, proposals)
        ...
        node_final_state = self.node_hn_mapping(sent_dict['hn'])
        node_final_state = node_final_state.repeat_interleave(repeats=roi_num, dim=0)
        node_features = self.roi_node_mapping(roi_feature)
        
        # print(node_features.shape, node_final_state.shape)
        graph_weights_logits, graph_weights = self.compute_graph_edge_weight_batch(node_features.reshape(N, -1, self.roi_hidden_dim), node_final_state.reshape(N, -1, self.roi_hidden_dim), roi_num)
        ...
        adj = graph_weights_logits.repeat((1, 1, roi_num)).reshape(N, roi_num, roi_num) # [N, roi_num, roi_num]
        
        
        node_features = node_features.reshape(N, -1, self.graph_in_channels) # [N, roi_num, roi_hidden_dim]
        node_features = node_features + self.graphNet(node_features, adj=adj if not self.woadj else None)
    ...
    def forward(self, batched_inputs):
        ...
        roi_feature = self.roi_box_feature_extractor(features, proposals)  # [N*B, 256, vh, vw]
        ...
        if not self.use_gcn:
            ...
            # print(roi_feature.shape)
            # print(self.roi_heads.box_predictor)
            # assert 1 == 0
            _, roi_loss = self.roi_excute(roi_feature, features, proposals, image_bin=image_bin)
        ...
        # compute the grpah weight
        # print(node_features.shape, final_state.shape)
        # assert 1 == 0
        node_final_state = self.node_hn_mapping(sent_dict['hn'])
        node_final_state = node_final_state.repeat_interleave(repeats=roi_num, dim=0)
        node_features = self.roi_node_mapping(roi_feature)
        ...
        # aligment loss
        contrastive_loss = info_nce(graph_weights_logits, contrastive_gt)  
        ...
        for step in range(self.Rstep):
            ...
            node_features = node_features + self.graphNets[step](node_features, adj=adj)
            node_features = self.gcn_bn_relu[step](node_features.reshape(-1, self.graph_in_channels)).reshape(N, roi_num, self.graph_in_channels)
