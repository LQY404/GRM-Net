...
class MGCN(nn.Module):
    ...
    def __init__(
        ...
        self.time = 0
        self.save_path = "/home/lingpeng/project/Adet_2022/effe_RPN_pred_in_train/" + cfg.RPN_SAVE + "_gcn" + ("2" if self.test2 else "") + ("_use_film" if self.use_film else "")
        os.makedirs(self.save_path, exist_ok=True)
        
        self.save_path_test = "/home/lingpeng/project/Adet_2022/effe_RPN_pred_in_test/"  + cfg.RPN_SAVE + "_gcn" + ("2" if self.test2 else "") + ("_use_film" if self.use_film else "")
    ...
    def forward(self, batched_inputs):
        ...
        '''
        # 直接忽略后面的GCN以及contrastive aligment loss的计算过程
        ...
        # t1 = time.time()
        node_features = node_features + self.graphNet(node_features, adj=adj)  # 同样要改
