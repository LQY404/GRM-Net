...
class DefaultAnchorGenerator(nn.Module):
    ...
    def generate_cell_anchors(self, sizes=(32, 64, 128, 256, 512), aspect_ratios=(0.5, 1, 2)):
        ...
        for size in sizes:
            area = size ** 2.0
            for aspect_ratio in aspect_ratios:  # [0.5, 1.0, 2.0]
                # s * s = w * h
                # a = h / w
                # ... some algebra ...
                # w = sqrt(s * s / a)
    ...
    def forward(self, features: List[torch.Tensor]):
        ...
        """
        grid_sizes = [feature_map.shape[-2:] for feature_map in features]  # 卷积后P2-P6的尺寸大小
        anchors_over_all_feature_maps = self._grid_anchors(grid_sizes)  #  把每个feature map上的anchor映射回原图

        # print("#" * 20)
        # print("输出anchor信息")
        # if type(anchors_over_all_feature_maps) == list:
        #     print(anchors_over_all_feature_maps[-1].shape)
        # else:
        #     print(anchors_over_all_feature_maps.shape)
