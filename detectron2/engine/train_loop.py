...
class SimpleTrainer(TrainerBase):
    ...
    def run_step(self):
        ...
        if accumulation_steps > 1:
            ...
            if (self.iter+1) % accumulation_steps == 0 or self.iter == self.max_iter:
                self.optimizer.step()
    ...
    def _write_metrics(self, loss_dict: Dict[str, torch.Tensor], data_time: float):
        ...
        if comm.is_main_process():
            ...
            # for name, param in self.model.named_parameters():
            #     storage.put_histogram(name+"_grad", param.grad)
                # storage.put_histogram(name+"_data", param)
